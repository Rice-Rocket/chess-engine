...
impl<'a> Evaluation<'a> {
    ...
    pub fn mobility_mg<W: Color, B: Color>(&self) -> i32 {
        ...
        while knights.0 != 0 {
            ...
            v += Self::MOBILITY_BONUS_MG[0][mobility[sqr] as usize];
        }

        while bishops.0 != 0 {
            let sqr = Coord::from_idx(bishops.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_MG[1][mobility[sqr] as usize];
        }

        while rooks.0 != 0 {
            let sqr = Coord::from_idx(rooks.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_MG[2][mobility[sqr] as usize];
        }

        while queens.0 != 0 {
            let sqr = Coord::from_idx(queens.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_MG[3][mobility[sqr] as usize];
        }
    ...
    pub fn mobility_eg<W: Color, B: Color>(&self) -> i32 {
        let mut knights = self.board.piece_bitboards[W::piece(Piece::KNIGHT)];
        let mut bishops = self.board.piece_bitboards[W::piece(Piece::BISHOP)];
        let mut rooks = self.board.piece_bitboards[W::piece(Piece::ROOK)];
        let mut queens = self.board.piece_bitboards[W::piece(Piece::QUEEN)];
        ...
        while knights.0 != 0 {
            let sqr = Coord::from_idx(knights.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_EG[0][mobility[sqr] as usize];
        }

        while bishops.0 != 0 {
            let sqr = Coord::from_idx(bishops.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_EG[1][mobility[sqr] as usize];
        }

        while rooks.0 != 0 {
            let sqr = Coord::from_idx(rooks.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_EG[2][mobility[sqr] as usize];
        }

        while queens.0 != 0 {
            let sqr = Coord::from_idx(queens.pop_lsb() as i8);
            v += Self::MOBILITY_BONUS_EG[3][mobility[sqr] as usize];
        }
...
mod tests {
    ...
    #[evaluation_test("1r3q1R/p1p1n2n/n2k1pR1/pQ3P1B/1bP2qpr/QP3n1P/P1P1P3/2B1N1RK w kq - 9 6")]
    fn test_mobility_mg() {
        assert_eval!(- mobility_mg, 193, 158, eval);
    }
}
