...
impl MagicBitBoards {
    ...
    pub fn get_rook_attacks(&self, square: Coord, blockers: BitBoard) -> BitBoard {
        let masked = (blockers & self.rook_mask[square]).0;
        let key = (masked.wrapping_mul(Self::ROOK_MAGICS[square])) >> (Self::ROOK_SHIFTS[square]);
        self.rook_attacks[square][key as usize]
    }

    pub fn get_bishop_attacks(&self, square: Coord, blockers: BitBoard) -> BitBoard {
        let masked = (blockers & self.bishop_mask[square]).0;
        let key = (masked.wrapping_mul(Self::BISHOP_MAGICS[square])) >> (Self::BISHOP_SHIFTS[square]);
...
impl Default for MagicBitBoards {
    fn default() -> Self {
        ...
        for sqr in Coord::iter_squares() {
            rook_attacks[sqr] = Self::create_table(sqr, true, Self::ROOK_MAGICS[sqr], Self::ROOK_SHIFTS[sqr]);
            bishop_attacks[sqr] = Self::create_table(sqr, false, Self::BISHOP_MAGICS[sqr], Self::BISHOP_SHIFTS[sqr]);
        }
