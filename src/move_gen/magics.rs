...
impl MagicBitBoards {
    ...
    pub fn get_slider_attacks(&self, square: Coord, blockers: u64, ortho: bool) -> u64 {
        if ortho { self.get_rook_attacks(square, blockers) } else { self.get_bishop_attacks(square, blockers) }
    }
    pub fn get_rook_attacks(&self, square: Coord, blockers: u64) -> u64 {
        let key = ((blockers & self.rook_mask[square.index()]) as u128 * Self::ROOK_MAGICS[square.index()] as u128) as u64 >> Self::ROOK_SHIFTS[square.index()];
        return self.rook_attacks[square.index()][key as usize];
    }
    pub fn get_bishop_attacks(&self, square: Coord, blockers: u64) -> u64 {
        let key = ((blockers & self.bishop_mask[square.index()]) as u128 * Self::BISHOP_MAGICS[square.index()] as u128) as u64 >> Self::BISHOP_SHIFTS[square.index()];
        return self.bishop_attacks[square.index()][key as usize];
    }

    pub fn create_blocker_bitboards(move_mask: u64) -> Vec<u64> {
        ...
        let mut blocker_bitboards = vec![0; n_patterns];

        for pattern_idx in 0..n_patterns {
            for bit_idx in 0..move_sqr_idx.len() {
                let bit = (pattern_idx >> bit_idx) & 1;
                blocker_bitboards[pattern_idx] |= (bit as u64) << move_sqr_idx[bit_idx];
            };
        };
        return blocker_bitboards
    }
    pub fn create_movement_mask(start_coord: Coord, ortho: bool) -> u64 {
        let mut mask = 0;
        ...
        for dir in directions {
            for dst in 1..8 {
                ...
                if next_coord.is_valid() {
                    BitBoardUtils::set_square(&mut mask, coord.square());
                } else { break; }
            }
        }
        return mask;
    }
    pub fn legal_move_bitboard_from_blockers(start_sqr: Coord, blockers: u64, ortho: bool) -> u64 {
        let mut bitboard = 0;
        let directions = if ortho { Coord::ROOK_DIRECTIONS } else { Coord::BISHOP_DIRECTIONS };
        ...
        for dir in directions {
            for dst in 1..8 {
                ...
                if coord.is_valid() {
                    BitBoardUtils::set_square(&mut bitboard, coord.square());
                    if BitBoardUtils::contains_square(&blockers, coord.square()) {
                        break;
                    }
                } else { break; }
            }
    ...
    //     return table;
    // }
    fn create_table(square: Coord, ortho: bool, magic: u64, left_shift: u32) -> Vec<u64> {
        let n_bits = 64 - left_shift;
        let lookup_size = 1 << n_bits;
        let mut table = vec![0; lookup_size];
        let move_mask = Self::create_movement_mask(square, ortho);
        let blockers = Self::create_blocker_bitboards(move_mask);

        for pattern in blockers {
            let index = (pattern as u128 * magic as u128) as u64 >> left_shift;
...
impl Default for MagicBitBoards {
    fn default() -> Self {
        ...
        let mut rook_attacks: [Vec<u64>; 64] = [EMPTY_VEC; 64];
        let mut bishop_attacks: [Vec<u64>; 64] = [EMPTY_VEC; 64];
