...
impl PseudoLegalMoveGenerator {
    ...
    pub fn sqr_attacked(&self, board: &Board, precomp: &Res<PrecomputedMoveData>, attack_sqr: u32, attacker_color: u8) -> bool {
        ...
        for i in 0..knight_attack_sqrs.len() {
            if board.square[knight_attack_sqrs[i] as usize].value() == (Piece::KNIGHT | attacker_color) {
                return true;
            }
        };
        for i in 0..2 {
            ...
            if !is_capture && !self.sqr_attacked(board, precomp, self.friendly_king_sqr, self.opponent_color) {
                if (target_sqr == representation::F1 as u8 || target_sqr == representation::F8 as u8) && self.has_kingside_castle_right(board) {
                    if !self.sqr_attacked(board, precomp, target_sqr as u32, self.opponent_color) {
                        ...
                        if board.square[castle_kingside_sqr as usize] == Piece::NULL {
                    ...
                    if !self.sqr_attacked(board, precomp, target_sqr as u32, self.opponent_color) {
                        ...
                        if board.square[castle_queenside_sqr as usize] == Piece::NULL && board.square[castle_queenside_sqr as usize - 1] == Piece::NULL {
